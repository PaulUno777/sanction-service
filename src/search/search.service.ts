...
export class SearchService {
  ...
  async searchfiltered(body: SearchParamDto) {
    ...
    if (typeof body.fullName != 'string')
      throw new BadRequestException('You must provide a fullname to search');
    ...
    if (typeof body.sanctionId == 'string' && body.sanctionId.length == 24) {
      sanctionedPipeline.push({
        $match: {
          $expr: {
            $eq: ['$listId', { $toObjectId: body.sanctionId }],
          },
        },
      });
      akaPipeline.push({
        $match: {
          $expr: {
            $eq: ['$sanctioned.0.listId', { $toObjectId: body.sanctionId }],
          },
        },
      });
    }
    ...
    if (typeof body.type == 'string') {
      ...
      akaPipeline.push({
        $match: {
          $expr: {
            $eq: [{ $strcasecmp: ['$sanctioned.0.type', body.type] }, 0],
          },
        },
      });
    }
